...
// αν θες να εμφανίζεται οπτικά το GeoJSON της στεριάς, βάλε true.
// Για απόδοση το κρατάμε false (το χρησιμοποιούμε ΜΟΝΟ για PIP).
...
// πιο “ελαφρύ” buffer γύρω από στεριά (deg)
const CLEARANCE_DEG = 0.06;
// ανώτατο πλήθος κελιών ανά leg (clamp για απόδοση)
...
function buildGridForBounds(
  ...
  // clamp συνολικών κελιών για απόδοση
  ...
  // clearance γύρω από στεριά (deg -> cells)
  ...
  return (
    <div className="w-full h-[420px] overflow-hidden rounded-2xl border border-slate-200 relative">
      ...
      <MapContainer
        ...
        preferCanvas={true}    // ✅ Canvas renderer για απόδοση
      ...
      >
        ...
        {/* στεριά — δεν τη ζωγραφίζουμε (μόνο PIP) για απόδοση */}
        <Pane name="pane-land" style={{ zIndex: 310 }}>
          {coast && SHOW_LAND_POLYGONS && (
            <GeoJSON
              ...
              interactive={false}
              style={() => ({ color: "#0b1220", weight: 1.5, opacity: 1, fillColor: "#ffffff", fillOpacity: 1 })}
            />
          )}
        </Pane>
