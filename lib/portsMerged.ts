...
// Merge rules:
// - Διπλά: κρατάμε name/region/coords από canonical, ΕΝΩΝΟΥΜΕ ops πεδία από facts (vhf, marina,
//   anchorage, shelter, exposure, hazards, notes, sources).
// - Facts-only: μπαίνουν κανονικά. Για να φανούν στο AI/Map χρειάζονται coords -> GEO_OVERRIDES εδώ
//   ή lat/lon μέσα στο ίδιο το PortFact.
...
/* ===== GEO OVERRIDES (Sea Guide Vol.3 – pp.03–62) =====
   WGS84 (δεκαδικές). Προαιρετικό region για facts-only.
*/
const GEO_OVERRIDES: Record<string, { lat?: number; lon?: number; region?: string }> = {
  ...
  /* Zakynthos (Vol.3) */
  "Alykes Harbour (Zakynthos)":          { lat: 37.0868, lon: 20.7818, region: "Zakynthos" },
  ...
  "Marmari Bay":                     { lat: 36.9279, lon: 21.7057, region: "Messinia" },
